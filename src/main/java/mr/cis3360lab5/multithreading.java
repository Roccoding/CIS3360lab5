...
public class multithreading {
    ...
    private final static ExecutorService scheduler = Executors.newScheduledThreadPool(MAX_THREADS);
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        if(allocate_map()==1)
        {
            System.out.println("allocated BitSet");
        }
        else
        {
        ...
        //wait more than 100 ms for all threads to finish, allocate one more PID     
        //give plenty of extra time for threads to actually start
        ...
        /* added after
        //acutally test all PIDS lol
        for(i=0; i<100; i++)
        {
            if(Bits.et.get(i+MIN_PID)==true)
                System.out.println("not released "+(i+MIN_PID));
        }
        */
